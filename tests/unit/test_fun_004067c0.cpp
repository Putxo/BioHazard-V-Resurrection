#include "re5/recovered/fun_004067c0.hpp"
...
void test_fun_004067c0() {
    ...
    FUN_004067C0_SetServices(nullptr);
    FUN_004067C0();

    State no_dispatch_state{};
    FUN_004067C0_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_004067C0_SetServices(&no_dispatch_services);
    FUN_004067C0();
    assert(no_dispatch_state.dispatch_calls == 0U);

    State state{};
    FUN_004067C0_Services services{&state, dispatch_resource_group};
    FUN_004067C0_SetServices(&services);
    FUN_004067C0();

    assert(state.dispatch_calls == 1U);
    assert(state.message_index == -1);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\ViewerResource") == 0);
    assert(state.direct_table == 0x01567BA0U);
