#include "re5/recovered/fun_00406800.hpp"
...
void test_fun_00406800() {
    ...
    // Host-only safety exit: the original 27-byte wrapper has no such branch.
    FUN_00406800_SetServices(nullptr);
    FUN_00406800();

    // Host-only safety exit for an absent injectable dispatcher.
    State no_dispatch_state{};
    FUN_00406800_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_00406800_SetServices(&no_dispatch_services);
    FUN_00406800();
    assert(no_dispatch_state.dispatch_calls == 0U);

    State state{};
    FUN_00406800_Services services{&state, dispatch_resource_group};
    FUN_00406800_SetServices(&services);
    FUN_00406800();

    assert(state.dispatch_calls == 1U);
    assert(state.message_index == -1);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\CreditResource") == 0);
    assert(state.direct_table == 0x01567C00U);
