#include "re5/recovered/fun_004068d0.hpp"
...
void test_fun_004068d0() {
    ...
    FUN_004068D0_SetServices(nullptr);
    FUN_004068D0();

    State no_dispatch_state{};
    FUN_004068D0_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_004068D0_SetServices(&no_dispatch_services);
    FUN_004068D0();
    assert(no_dispatch_state.dispatch_calls == 0U);

    State state{};
    FUN_004068D0_Services services{&state, dispatch_resource_group};
    FUN_004068D0_SetServices(&services);
    FUN_004068D0();

    assert(state.dispatch_calls == 1U);
    assert(state.message_index == -1);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\BenchResource") == 0);
    assert(state.direct_table == 0x01567E90U);
