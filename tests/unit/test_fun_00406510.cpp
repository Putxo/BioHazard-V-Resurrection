...
void test_fun_00406510() {
    ...
    FUN_00406510_SetServices(nullptr);
    FUN_00406510(2U);

    State no_dispatch_state{};
    FUN_00406510_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_00406510_SetServices(&no_dispatch_services);
    FUN_00406510(4U);
    assert(no_dispatch_state.calls == 0U);

    State state{};
    FUN_00406510_Services services{&state, dispatch_resource_group};
    FUN_00406510_SetServices(&services);
    FUN_00406510(11U);

    assert(state.calls == 1U);
    assert(state.selector == 11U);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\ShowGameResource") == 0);
    assert(state.primary_table == 0x01567640U);
    assert(state.primary_count == 13U);
    assert(state.secondary_table == 0x015676A8U);
