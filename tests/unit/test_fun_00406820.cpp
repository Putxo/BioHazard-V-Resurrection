#include "re5/recovered/fun_00406820.hpp"
...
void test_fun_00406820() {
    ...
    FUN_00406820_SetServices(nullptr);
    FUN_00406820();

    // Missing host dispatch is another harness-only early exit.
    State no_dispatch_state{};
    FUN_00406820_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_00406820_SetServices(&no_dispatch_services);
    FUN_00406820();
    assert(no_dispatch_state.dispatch_calls == 0U);

    State state{};
    FUN_00406820_Services services{&state, dispatch_resource_group};
    FUN_00406820_SetServices(&services);
    FUN_00406820();

    assert(state.dispatch_calls == 1U);
    assert(state.message_index == -1);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\Credit2Resource") == 0);
    assert(state.direct_table == 0x01567C08U);
