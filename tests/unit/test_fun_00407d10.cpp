...
void test_fun_00407d10() {
    ...
    // Delegated native early exits: null/empty suffix yields a null temporary,
    // so the destination is left untouched.
    FUN_00407D10(untouched, nullptr);
    assert(untouched.pointer == untouched_block);
    assert(untouched_block->ref_count == 7U);

    FUN_00407D10(untouched, "");
    assert(untouched.pointer == untouched_block);
    assert(untouched_block->ref_count == 7U);

    // Portable-host constructor guard inherited from FUN_00402360.
    FUN_00407D10(untouched, "suffix");
    assert(untouched.pointer == untouched_block);
    assert(untouched_block->ref_count == 7U);
    ...
    // Empty destination native branch: adopt the temporary, increment it,
    // then destroy the temporary reference. Net stored refcount remains one.
    FUN_00402360_String empty{nullptr};
    FUN_00407D10(empty, "Chris");
    auto* adopted =
        static_cast<FUN_00402360_Block*>(empty.pointer);
    assert(adopted != nullptr);
    assert(adopted->ref_count == 1U);
    assert(adopted->length == 5U);
    assert(std::strcmp(adopted->data, "Chris") == 0);
    assert(strings.requested_size == 5U + 0x0CU);
    assert(strings.requested_alignment == 0x10U);

    // Harness-only missing-concatenate guard: an existing destination is
    // preserved while the already-constructed temporary is released.
    const unsigned frees_before_missing = strings.free_calls;
    FUN_00407D10(untouched, "+x");
    assert(untouched.pointer == untouched_block);
    assert(untouched_block->ref_count == 7U);
    assert(strings.free_calls == frees_before_missing + 1U);

    ConcatenateState concat{};
    FUN_00407D10_Services services{
        &concat,
        concatenate,
    };
    FUN_00407D10_SetServices(&services);

    // Existing shared destination: concatenate old+suffix, decrement the old
    // reference without freeing it, install the new refcount-1 block, then
    // release the temporary suffix.
    ...
    // Existing unique destination: release reaches zero and invokes the
    // allocator free path before the combined block is installed.
    ...
    // Harness-only concatenate/allocation-failure guard: old destination is
    // untouched and the temporary suffix is still released.
    ...
    // Portable-host temporary allocation failure also preserves destination.
    strings.return_null = true;
    FUN_00407D10(failure, "alloc-fail");
    assert(failure.pointer == failure_block);
    assert(failure_block->ref_count == 5U);
