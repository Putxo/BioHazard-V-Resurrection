...
void test_fun_00406600() {
    ...
    FUN_00406600_SetServices(nullptr);
    FUN_00406600();

    // Missing opaque dispatch is the second host-only early exit.
    State no_dispatch_state{};
    FUN_00406600_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_00406600_SetServices(&no_dispatch_services);
    FUN_00406600();
    assert(no_dispatch_state.dispatch_calls == 0U);

    State state{};
    FUN_00406600_Services services{&state, dispatch_resource_group};
    FUN_00406600_SetServices(&services);
    FUN_00406600();

    assert(state.dispatch_calls == 1U);
    assert(state.message_index == -1);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\MenuResource") == 0);
    assert(state.direct_table == 0x01567800U);
    assert(state.direct_count == 22U);
