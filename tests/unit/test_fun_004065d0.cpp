...
void test_fun_004065d0() {
    ...
    FUN_004065D0_SetServices(nullptr);
    FUN_004065D0();

    // Host-only early exit after the trace hook when dispatch is unavailable.
    State no_dispatch_state{};
    FUN_004065D0_Services no_dispatch_services{&no_dispatch_state, trace_marker, nullptr};
    FUN_004065D0_SetServices(&no_dispatch_services);
    FUN_004065D0();
    ...
    // A missing host trace hook must not suppress the native dispatch semantics.
    State no_trace_state{};
    FUN_004065D0_Services no_trace_services{&no_trace_state, nullptr, dispatch_resource_group};
    FUN_004065D0_SetServices(&no_trace_services);
    FUN_004065D0();
    assert(no_trace_state.trace_calls == 0U);
    assert(no_trace_state.dispatch_calls == 1U);
    assert(no_trace_state.message_index == -1);
    assert(no_trace_state.archive_name != nullptr);
    assert(std::strcmp(no_trace_state.archive_name, "Image\\Archive\\ShowTitleResource") == 0);
    assert(no_trace_state.direct_table == 0x015677D8U);
    assert(no_trace_state.direct_count == 5);
    assert(no_trace_state.resolved_table == 0U);
    assert(no_trace_state.resolved_count == 0);

    State state{};
    FUN_004065D0_Services services{&state, trace_marker, dispatch_resource_group};
    FUN_004065D0_SetServices(&services);
    FUN_004065D0();
    ...
    assert(std::strcmp(state.archive_name, "Image\\Archive\\ShowTitleResource") == 0);
    assert(state.direct_table == 0x015677D8U);
    assert(state.direct_count == 5);
