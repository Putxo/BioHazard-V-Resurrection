...
void test_fun_00406620() {
    ...
    FUN_00406620_SetServices(nullptr);
    FUN_00406620();

    // Missing host dispatch is another harness-only early exit.
    State no_dispatch_state{};
    FUN_00406620_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_00406620_SetServices(&no_dispatch_services);
    FUN_00406620();
    assert(no_dispatch_state.dispatch_calls == 0U);

    State state{};
    FUN_00406620_Services services{&state, dispatch_resource_group};
    FUN_00406620_SetServices(&services);
    FUN_00406620();

    assert(state.dispatch_calls == 1U);
    assert(state.message_index == -1);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\Menu2Resource") == 0);
    assert(state.direct_table == 0x015678B0U);
