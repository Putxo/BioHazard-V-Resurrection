#include "re5/recovered/fun_004066c0.hpp"
...
void test_fun_004066c0() {
    ...
    FUN_004066C0_SetServices(nullptr);
    FUN_004066C0();

    // Missing host dispatch is another harness-only early exit.
    State no_dispatch_state{};
    FUN_004066C0_Services no_dispatch_services{&no_dispatch_state, nullptr};
    FUN_004066C0_SetServices(&no_dispatch_services);
    FUN_004066C0();
    assert(no_dispatch_state.dispatch_calls == 0U);

    State state{};
    FUN_004066C0_Services services{&state, dispatch_resource_group};
    FUN_004066C0_SetServices(&services);
    FUN_004066C0();

    assert(state.dispatch_calls == 1U);
    assert(state.message_index == -1);
    assert(state.archive_name != nullptr);
    assert(std::strcmp(state.archive_name, "Image\\Archive\\VersusResource") == 0);
    assert(state.direct_table == 0x01567958U);
    assert(state.direct_count == 9);
