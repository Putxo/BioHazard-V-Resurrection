...
def load():
    ...
    return out
def registry(sel):
    rows=list(csv.DictReader((ROOT/'database/functions.csv').open()));by={r['stable_id']:r for r in rows};claims=list(csv.DictReader((ROOT/'database/claims.csv').open()));req(len(rows)==1000 and len(by)==1000,'registry must be exactly 1000 unique rows')
    for e in sel:
    ...
    print('PASS registry: 1000 / 79782; 800 existing leaf implementations bound once each')
