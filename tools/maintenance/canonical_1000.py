...
SEL=ROOT/'decomp/evidence/canonical_1000_selected.csv'
...
def selection():
    ids=[r['stable_id'] for r in csv.DictReader(SEL.open())]; req(len(ids)==800 and len(set(ids))==800,'need 800 unique ids')
...
def refresh(publish=False):
    ...
    subprocess.run([sys.executable,'tools/verification/verify_canonical_1000.py','--registry'],cwd=ROOT,check=True)
def note(e):
    f=e['family']
...
def register():
    before=rows(DB[0]);req(len(before)==200,'expected 200-entry base');existing={r['stable_id'] for r in before};sel=selection();req(not existing&{e['symbol'] for e in sel},'target already registered')
    catalog_list=json.loads(CAT.read_text())['records']; index={r['symbol']:i for i,r in enumerate(catalog_list)}
    fs=[];cs=[];hs=[]
    for e in sel:
        sid=e['symbol'];cid='CANONICAL-1000-'+sid[4:];va=e['va'];src='decomp/leaf323d1aab/generated/src/leaves_%03d.cpp'%(index[sid]//256)
        fs.append(dict(stable_id=sid,va=f'0x{va:08X}',rva=f"0x{e['rva']:08X}",section='.text',size=str(e['size']),provisional_name=sid,workflow_status='ACTIVE',confidence='HIGH',fast_pass='true',fast_pass_validated='false',converted='false',refined='false',verified='false',matched='false',linked='false',calling_convention='thiscall',return_type='uint32_t',arguments='',this_type='const void*; observed PE32 leaf contract',module='UNKNOWN',subsystem='LeafRecovery',source_file=src,owner='coordinator',claim_id=cid,pr='',commit='',test='decomp/leaf323d1aab/tests/properties.cpp',evidence='decomp/evidence/canonical_1000.md',notes=note(e)))
        cs.append(dict(claim_id=cid,stable_id=sid,owner='coordinator',branch='decomp/canonical-1000',task='Register existing exact-target leaf after frozen V6 membership and byte-identical I386 revalidation',claimed_at='2026-09-26T00:24:00+02:00',expires_at='',pr='',active='true'))
    ...
    cm=ROOT/'CMakeLists.txt';inc='include(cmake/canonical_1000.cmake)';req(inc not in cm.read_text(),'already attached');cm.write_text(cm.read_text()+'\n# Official 1,000-entry registry validation for existing recovered leaves.\n'+inc+'\n')
    refresh(False)
def promote(pr,head,merge):
    data=rows(DB[0]);req(len(data)==1000,'expected 1000-entry registration');ids={e['symbol'] for e in selection()};by={r['stable_id']:r for r in data};changes={}
    for sid in ids:
    ...
    receipt={'recovery_pr':pr,'recovery_head':head,'recovery_merge':merge,'validated_count':800,'tier':'FAST_PASS_VALIDATED','denominator':79782,'game_linked':False};(ROOT/'decomp/evidence/canonical_1000_promotion.json').write_text(json.dumps(receipt,indent=2)+'\n');refresh(False)
def main():
