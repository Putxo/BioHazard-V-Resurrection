...
def baseline():
    if not FREEZE.exists(): return {'total':0,'total_bytes':0}
    d=json.loads(FREEZE.read_text(encoding='utf-8'))
    return {'total':int(d.get('function_candidate_denominator',0)), 'total_bytes':int(d.get('estimated_function_bytes',0))}
...
def metrics(rs):
    b=baseline(); total=b['total'] or len(rs); total_bytes=b['total_bytes'] or sum(int(r['size'],0) for r in rs if r.get('size'))
...
def progress_block(m):
    functions=bar(m['functions_discovered'],m['total']); code_size=bar(m['bytes_discovered'],m['total_bytes']) if m['total_bytes'] else '░'*30+'  pending'
    return f'```\nFunctions   {functions}\nCode size   {code_size}\n```'
def tiers_block(m):
    lines=['```']
