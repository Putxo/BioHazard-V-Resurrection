#!/usr/bin/env python3
"""Inspect actual Windows I386 object definitions; never publish object files."""
...
def inspect(path):
    ...
    while i<nsym:
        ...
        name=name.rstrip(b'\0').decode('ascii',errors='replace');value,section,typ,storage,aux=struct.unpack_from('<IhHBB',data,s+8)
        ...
        i+=1+aux
    rows=records()
    if set(names)!={r['symbol'] for r in rows}:raise ValueError('Missing or extra named definition')
    exact=[]
    for r in rows:
    ...
    return {'machine':'I386','named_definitions':len(names),'exact_bodies':len(exact),'exact_instruction_bytes':sum(r['size'] for r in exact),'object_sha256':hashlib.sha256(data).hexdigest(),'game_linked':False}
