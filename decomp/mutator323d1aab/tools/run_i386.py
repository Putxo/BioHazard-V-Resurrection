#!/usr/bin/env python3
"""Build/run isolated source and original-hash instruction fixtures; never launch the PE."""
...
def main():
    ...
    try:r=subprocess.run([str(exe)],check=False,capture_output=True,text=True,timeout=60)
    ...
    for label in ('i386-source','i386-hash-checked-reference'):
        if f'PASS {label} 605098 checks; 422 bodies' not in r.stdout:raise ValueError('Missing complete fixture result')
